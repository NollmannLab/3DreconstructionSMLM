...
%save the variable 
Folder = uigetdir('Choose path to write output data');
...
% Generate Spirale coordinates
t = 22:0.01:11.56*pi;
x = 0.8*exp(0.125*t).*sin(0.9*t);
y = 0.8*exp(0.125*t).*cos(0.9*t);
z = 0.09*exp(0.2*t);

len=sum( sqrt( sum( diff([x;y;z]',1,1).^2 ,2)) ,1);
fprintf('\n Structure length is =  %.0f \n',len)


% Generate volume and dilate it with a ball of 10nm
VoxSize = 1; % 1nm
Vol = Coord2Vol(y,x,z,[200 200 200],[VoxSize VoxSize VoxSize]);
se=strel(ones(10,10,10)); %to get a width of 10nm = 10pix
Vol2 = imdilate(Vol,se);
% figure(1)
% imagesc(rot90(squeeze(max(Vol2,[],2))))

%% Convert back to coordinates (imresize only works on images)
[row,col,slice] = ind2sub(size(Vol2),find(Vol2>0));
clear x y z
x = VoxSize*row;
y = VoxSize*col;
z = VoxSize*slice;
...
% And convert it back to a volume with the desired voxel size (Model)
% This is twisted but I can't work with too many loclaisation as the
% distance matrix (see later) takes lots of memory
Vol = Coord2Vol(x,y,z,[100 100 100],[5 5 5]);
% figure(4)
% imagesc(rot90(squeeze(max(Vol,[],2))))

%% From the model I generate localisations
[row,col,slice] = ind2sub(size(Vol),find(Vol>0));
clear X_t Y_t Z_t
X_t = 5*row';
Y_t = 5*col';
Z_t = 5*slice';
