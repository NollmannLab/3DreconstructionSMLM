...
%% reading geometry
 path1 = strrep(which('SimT4_Main'),['SimT4_Main' '.m'],'');
 cd(path1);
 Ima = im2double(imread(strcat(path1,'bacteria-ext8bitSimp-1 - Antoine-2.tif')));

 parameters.planes = 360;
 Ima2 = [];
 
 Ima2 = im2double(imread(strcat(path1,'bacteria-ext8bitSimp-2 - Antoine-1.tif')));
     
     % determine how many time the plane is repite it
     dlg_title = 'Planes';
     num_lines = 1;
     def = {'6','360'};
     prompt = {'Number of planes','Total n� of planes'};
        
     answer = inputdlg(prompt,dlg_title,num_lines,def);
     parameters.planes = str2double(answer{1});
     parameters.planes_rotation = str2double(answer{2});

 
%save the variable 
Folder = uigetdir(path1,'Choose path to write output data');
...
%% determine all possibilities of points localizations
Ima1 = Ima;

[X_t,Y_t,Z_t,x1,x2,y1,y2,z1,z2]=geometry_allPoints(Ima1,Ima2,parameters); %  seems to be in nm
disp(['Total number of labeling sites = ' num2str(numel(X_t))]) ;
fprintf('\n Nb of emitters for the trunc+head = %4.f \n',numel(x1))
fprintf('\n Nb of emitters for the legs = %4.f \n',numel(x2))
...
for k=1:N_Clust 
    ...
    [cluster]=geometry_SelPoints_v2(X_t,Y_t,Z_t,x1,x2,y1,y2,z1,z2,parameters);
